...
<!-- main content -->
			<div id="left">
                <h1>สถานะการลงทะเบียนข้อมูล ของ NU</h1>
...
echo"<CENTER>";
echo"<Form method='post' action='?name=admin&file=data_all_nu'> <Table border=0>
...
if($num_rows>10)
{
	If ($Checkpage>1)		//ถ้าแสดงหน้าที่มากกว่า 1 จะมี Previous
		{
		echo "[ <A Href=".$Scriptname."?name=admin&file=data_all_nu&Page=".($Checkpage-1)."&Pagesize=".$Pagesize.">กลับไปหน้าที่แล้ว</A> ]";
		}
	echo "[";
	$Pad="";
	...
	for ($N=1; $N<=$Maxcount; $N=$N+1)
	{
		...
		$Ref="<A Href=".$Scriptname."?name=admin&file=data_all_nu&Page=".$N."&Pagesize=".$Pagesize.">".$Pad.$N."</A>";
		echo "  ".$Ref."  ";
	} 
	echo "]";
If ($Checkpage<$Maxcount)		//ถ้าแสดงหน้าที่น้อยกว่าหน้าสูงสุด จะมี Next
	{
	echo "[<A Href=".$Scriptname."?name=admin&file=data_all_nu&Page=".($Checkpage+1)."&Pagesize=".$Pagesize.">หน้าถัดไป</A> ]";
	}
echo"</CENTER>";
}
echo "</Table>";
...
?>
		  ...
		  <h3 align="center">0 = No Data, 1 = Incomplete  ,  2 = Complete  ,  3=  Approve </h3>
          	 ...
          	 <?
 		if($codehos == "NU"){
		echo "<a href='report/excel-nu.php'><font color=\"red\"><b>	REPORT  EXCEL  DOWNLOAD </b></font></a>";
		} 		
	?>	
