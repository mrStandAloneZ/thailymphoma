...
$checkLogin = "select * from " . TB_MEMBER . " where user='" . $_SESSION['login_true'] . "'";
$result = mysql_query($checkLogin) or die("Err Can not to result");
...
<tr>
  ...
  <td colspan="3" ><br />
   <?php if ($dbarr['payment']) {echo $dbarr['payment'];} else {echo "-";}?>
...
<tr>
  ...
  <td colspan="3" > <br />
    <?php if ($dbarr['ext_none']) {echo "None ";}?>
    <?php if ($dbarr['ext_wal']) {echo "Waldeyer's ring ";}?>
    <?php if ($dbarr['ext_sin']) {echo "Sinonasal ";}?>
    <?php if ($dbarr['ext_sal']) {echo "Salivary gland ";}?>
    <?php if ($dbarr['ext_thy']) {echo "Thyroid ";}?>
    <?php if ($dbarr['ext_eye']) {echo "Eye/Ocular adnexa ";}?>
    <?php if ($dbarr['ext_lung']) {echo "Lung/Pleura ";}?>
    <?php if ($dbarr['ext_breast']) {echo "Breast ";}?>
    <?php if ($dbarr['ext_stomach']) {echo "Stomach ";}?>
    <?php if ($dbarr['ext_small']) {echo "Small intestine ";}?>
    <?php if ($dbarr['ext_testis']) {echo "Testis ";}?>
    <?php if ($dbarr['ext_brain']) {echo "Brain/CNS ";}?>
    <?php if ($dbarr['ext_liver']) {echo "Liver ";}?>
    <?php if ($dbarr['ext_large']) {echo "Large intestine ";}?>
    <?php if ($dbarr['ext_bone']) {echo "Bone marrow ";}?>
    <?php if ($dbarr['ext_spleen']) {echo "Spleen ";}?>
    <?php if ($dbarr['ext_skin']) {echo "Skin/Subcutaneous ";}?>
    <?php if ($dbarr['ext_other']) {echo "Others: " . $dbarr['ext_other_text'];}?>
      <br /><br />
    </td>
  </tr>
...
<tr>
  ...
  <td colspan="3" > <br />
    <?php echo $dbarr['date_record_follow']; ?><br /><br /></td>
</tr>
<tr>
   ...
   <script type="text/javascript">
    ...
    function calyear(){
      ...
      if(birth != ""){
        var res = birth.split("-");
        var date = new Date();
        var year = (date.getFullYear() + 543)*365 + ((1+date.getMonth())*30) +date.getDate();
