...
echo"<CENTER>";
echo"<Form method='post' action='?name=member&file=third_lind_tr'> <Table border=0>
...
While  ($Recs<=$Pagesize)	// แสดงเรกคอร์ดจำนวนไม่เกินจำนวนเรกคอร์ดต่อหน้า
	{		
			if($count<$numrows)
		   {
				...
				$status_third=	 $result[status_third];
			If ($bg=="skyblue")
				{
				$bg="skyblue";		
				}
	...
	echo "<Tr bgcolor=$bg>                   
			   ...
			   <Td><center>$diagnosis_cml</center></Td>      
   			   <Td><center>$payment</center></Td>";  ?> 
			    <?		if ($status_third == 3){		//  ส่วนของการเปรียบเทียบข้อมูล ว่าตอนนี้อยู่ในสถานะไหน ถ้าส่งข้อมูลให้ส่วนกลางแล้วไม่สามารถ ให้แก้ไขข้อมูลได้ครับ			
			 echo" <td><center>
							No Edit</center>
			</td> ";
				}
					...
					echo "  <td><center>
								<A href='index.php?name=member&file=edit_data_pfour&id=$id'>Edit</center></a>
						</td> ";
				}  ?>
...
if($num_rows>10)
{
	If ($Checkpage>1)		//ถ้าแสดงหน้าที่มากกว่า 1 จะมี Previous
		{
		echo "[ <A Href=".$Scriptname."?name=member&file=third_lind_tr&Page=".($Checkpage-1)."&Pagesize=".$Pagesize.">กลับไปหน้าที่แล้ว</A> ]";
		}
	echo "[";
	$Pad="";
	...
	for ($N=1; $N<=$Maxcount; $N=$N+1)
	{
		...
		$Ref="<A Href=".$Scriptname."?name=member&file=third_lind_tr&Page=".$N."&Pagesize=".$Pagesize.">".$Pad.$N."</A>";
		echo "  ".$Ref."  ";
	} 
	echo "]";
If ($Checkpage<$Maxcount)		//ถ้าแสดงหน้าที่น้อยกว่าหน้าสูงสุด จะมี Next
	{
	echo "[<A Href=".$Scriptname."?name=member&file=third_lind_tr&Page=".($Checkpage+1)."&Pagesize=".$Pagesize.">หน้าถัดไป</A> ]";
	}
echo"</CENTER>";
}
echo "</Table>";
