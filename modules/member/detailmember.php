...
echo"<CENTER><Br>";
echo"<Form method='post' action='index.php?name=member&file=detailmember'>                              
...
{
	$sql2 = " where status='อนุมัติ' and $field_search like '%$search%'";
	$set=1;
}
?>
<? } else{}?>
...
// คำสั่ง SQL และสั่งให้ทำงาน
	$sql = "select * from $tblname where member_id='$bb' and status='อนุมัติ'  ";
}    elseif  ($dd =='200'){
	$sql = "select * from $tblname where adviser_id='$cc' and status='อนุมัติ' ";
}
	  elseif ($dd =='300') {
	$sql = "select * from $tblname where typepa='$rr' and status='หัวหน้าภาคอนุมัติ' ";
}
     else {
		$sql = "select * from $tblname where  status='เจ้าหน้าที่วิชาการอนุมัติ'   ";
}
...
if($set==1)
{
	$sql2 = "select * from $tblname where status='อนุมัติ' and $field_search like '%$search%' order by id limit  $Gopage,$Pagesize";	
	echo "<Font color=blue> แสดงผลการค้นหาข้อมูลที่มีคำว่า </Font><Font color=red>".$search." </Font><Font  color=blue>ที่ได้รับการ <Font color=red>อนุมัติ </Font><br>&nbsp;";
}
else
{
if($dd =='100' )
{
$sql2 = "select * from $tblname  where member_id='$bb'  and status='อนุมัติ' order by id limit  $Gopage,$Pagesize";	 
}
	elseif ($dd =='200') {
		$sql2 = "select * from $tblname  where adviser_id='$cc' and status='อนุมัติ' order by id limit  $Gopage,$Pagesize";	 
	}
	elseif ($dd =='300'){
		$sql2 = "select * from $tblname  where typepa='$rr'  and status='หัวหน้าภาคอนุมัติ' order by id limit  $Gopage,$Pagesize";	 
	}
else {
	$sql2 = "select * from $tblname  where  status='เจ้าหน้าที่วิชาการอนุมัติ'  order by id limit  $Gopage,$Pagesize";	 
...
<Td>สถานะ</Td>                                                 <Td>รายละเอียด</Td>							<td>print</td>					                 ";  
?>
<? } else { ?>
<?
$bg="pink";	//กำหนดสีเริ่มต้น
   ...
   if($dd !='200' || $dd !='300' || $dd !='400'){
	...
	echo "<Tr bgcolor=$bg>                   
			   ...
			   <Td><center><A Href='index.php?name=member&file=title_old&id=$id'>ดู</center></A></Td>      
			     <Td><center><A Href='report/print_show?id=$id'>print</center></A></Td>      
