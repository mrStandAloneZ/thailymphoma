...
<TABLE width="90%" align=center cellSpacing=0 cellPadding=0 border=0>
				<TR>
					...
					<A HREF="?name=admin&file=knowledge"><IMG SRC="images/admin/open.gif"  BORDER="0" align="absmiddle"> รายการกิจกรรมโรงพยาบาล</A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=knowledge&op=article_add"><IMG SRC="images/admin/book.gif"  BORDER="0" align="absmiddle"> เพิ่มกิจกรรมโรงพยาบาล</A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=knowledge_category"><IMG SRC="images/admin/folders.gif"  BORDER="0" align="absmiddle"> รายการหมวดหมู่</A> &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=knowledge_category&op=newscat_add"><IMG SRC="images/admin/opendir.gif"  BORDER="0" align="absmiddle"> เพิ่มหมวดหมู่</A><BR><BR>
<?
//////////////////////////////////////////// แสดงรายการ
if($_GET[op] == ""){
?>
...
?>
    <tr>
     <td width="44">
      <a href="?name=admin&file=knowledge_category&op=newscat_edit&id=<? echo $arr[knowledgecat][id];?>"><img src="images/admin/edit.gif" border="0" alt="แก้ไข" ></a> 
      <a href="javascript:Confirm('?name=admin&file=knowledge_category&op=newscat_del&id=<? echo $arr[knowledgecat][id];?>','คุณมั่นใจในการลบหมวดหมู่นี้ ?');"><img src="images/admin/trash.gif"  border="0" alt="ลบ" ></a>
     </td> 
     <td><?echo $arr[knowledgecat][category_name];?></td>
	 <td align="center" width="50" ><?echo $row[sumknowledge] ;?></td>
     <td align="center" width="50"><A HREF="?name=admin&file=knowledge_category&op=newscat_edit&action=sort&setsort=<?echo $SETSORT_UP ;?>&move=up&id=<? echo $arr[knowledgecat][id];?>"><IMG SRC="images/icon/arrow_up.gif"  BORDER="0" ALT="เลื่อนขึ้น"></A>&nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=knowledge_category&op=newscat_edit&action=sort&setsort=<?echo $SETSORT_DOWN ;?>&move=down&id=<? echo $arr[knowledgecat][id];?>"><IMG SRC="images/icon/arrow_down.gif"  BORDER="0" ALT="เลื่อนลง"></A></td>
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการเพิ่มหมวดหมู่กิจกรรมโรงพยาบาล   เข้าสู่ระบบเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=knowledge_category\"><B>กลับหน้า จัดการหมวดหมู่กิจกรรมโรงพยาบาล </B></A>";
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการแก้ไขหมวดหมู่กิจกรรมโรงพยาบาล   เข้าสู่ระบบเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=knowledge_category\"><B>กลับหน้า จัดการหมวดหมู่กิจกรรมโรงพยาบาล </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	}else{
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<BR><BR>";
		$ProcessOutput .= "<CENTER><A HREF=\"?name=admin&file=main\"><IMG SRC=\"images/login-redirection-loader.gif\" BORDER=\"0\"></A><BR><BR>";
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการลบหมวดหมู่กิจกรรมโรงพยาบาลเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=knowledge_category\"><B>กลับหน้า จัดการหมวดหมู่กิจกรรมโรงพยาบาล</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	}else{
