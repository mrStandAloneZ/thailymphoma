...
// header("Content-Disposition: attachment; filename=lymphoma-$date_save.xls");
?>
<html>
<body>
...
$strSQL = "SELECT * FROM web_add_data1  WHERE (dateofrecord BETWEEN '" . $mindate . "' AND '" . $maxdate . "')";
...
<th> <div align="center">Status Bulky</div></th>
...
while ($objResult = mysql_fetch_array($objQuery)) {
    ...
    $patient_initials = $objResult['patient_initials'];
	            ...
	            $id = $objResult['id'];
                $dateofrecord = $objResult['dateofrecord'];
                $subject = $objResult['subject'];
                $centre = $objResult['centre'];
                $d_record = $objResult['d_record'];
                $m_record = $objResult['m_record'];
                $y_record = $objResult['y_record'];
                $date_of_record = $objResult['date_of_record'];
                $patient_initials = $objResult['patient_initials'];
                $sex = $objResult['sex'];
                $id_card = $objResult['id_card'];
                $hn = $objResult['hn'];
                $date_of_birth = $objResult['date_of_birth'];
                $province = $objResult['province'];
                $payment = $objResult['payment'];
                $date_bio_report = $objResult['date_bio_report'];
                $pathology = $objResult['pathology'];
                $biopsy_site = $objResult['biopsy_site'];
                $type = $objResult['type'];
                $hodgkin_don = $objResult['hodgkin_don'];
                $type_non = $objResult['type_non'];
                $type_sub_non = $objResult['type_sub_non'];
                $who_sub = $objResult['who_sub'];
                $work_sub = $objResult['work_sub'];
                $other_type = $objResult['other_type'];
                $ann_arbor = $objResult['ann_arbor'];
                $symptom_ann = $objResult['symptom_ann'];
                $ext_none = $objResult['ext_none'];
                $ext_wal = $objResult['ext_wal'];
                $ext_sin = $objResult['ext_sin'];
                $ext_sal = $objResult['ext_sal'];
                $ext_thy = $objResult['ext_thy'];
                $ext_eye = $objResult['ext_eye'];
                $ext_lung = $objResult['ext_lung'];
                $ext_breast = $objResult['ext_breast'];
                $ext_stomach = $objResult['ext_stomach'];
                $ext_small = $objResult['ext_small'];
                $ext_testis = $objResult['ext_testis'];
                $ext_brain = $objResult['ext_brain'];
                $ext_liver = $objResult['ext_liver'];
                $ext_large = $objResult['ext_large'];
                $ext_bone = $objResult['ext_bone'];
                $ext_spleen = $objResult['ext_spleen'];
                $ext_skin = $objResult['ext_skin'];
                $ext_other = $objResult['ext_other'];
                $ext_other_text = $objResult['ext_other_text'];
                $per_ecog = $objResult['per_ecog'];
                $ldh = $objResult['ldh'];
                $hiv_positive = $objResult['hiv_positive'];
			 	$received_follow = $objResult["received_follow"];
				
				   $ntype_noe =  $objResult["type_non"]; 
				   
                   $nwho_sub =  trim($objResult["who_sub"]); 
                   $nwork_sub=  trim($objResult["work_sub"]); 
				   
				   if($nwho_sub == "" && $nwork_sub == "")
				     {$ntestsub="";}else{$ntestsub="yes";}
				   
	
    if($patient_initials != "" && $sex != "" && $id_card != "" && $hn != "" && $date_of_birth != "" && $province != "" && $payment != "" && $date_bio_report != "" && $pathology != "" && $biopsy_site != "" && $type != "" && $ann_arbor != "" && $symptom_ann != "" && $per_ecog != "" && $ldh != "" && $hiv_positive != "" &&  $ntype_noe!="" && $ntestsub !="" ) {
    ...
    <td align="right"><?php echo $objResult["date_bio_report"]; ?></td>
    <td align="right"><?php echo $objResult["pathology"]; ?></td>
    <td align="right"><?php echo $objResult["biopsy_site"]; ?> <? if ($objResult["biopsy_site"] == 'Others') { echo $objResult["oth_biopsy"];} ?></td>
...
$txtex= "";
    ...
    <td align="right">
      <?php if ($objResult['status_bulky'] == "yes") {
          echo "Yes";
      } else if ($objResult['status_bulky'] == "no") {
          echo "No";
      }
      ?></td>
...
$text_ipi = "Low";
    ...
    <?php ////***************************************************************?>
        ...
        <?php //   follow up
		   ...
		   //*******status2  follow
                $result_follow = $objResult['result_follow'];
                $result_cause_follow = $objResult['result_cause_follow'];
                $progression_follow = $objResult['progression_follow'];
                $salvage_follow = $objResult['salvage_follow'];
                $surgery_follow = $objResult['surgery_follow'];
                $stem_cell_follow = $objResult['stem_cell_follow'];
                $date_last_contact_follow = $objResult['date_last_contact_follow'];
                $status_follow = $objResult['status_follow'];
                ...
                if ($result_follow != "" && $progression_follow != "" && $salvage_follow != "" && $stem_cell_follow != "" && $date_last_contact_follow != "" && $status_follow != "" && $received_follow!="") {
                    $status2_follow2= "complete";
                } else if ($patient_initials != "") {
                    $status2_follow2 = "incomplete";
                } else {
                    $status2_follow2 = "No Data";
                }
		
		
		
		?>
        <td align="right"><?php echo $status2_follow2; //BE    ?></td>
    ...
    ?>
		...
		<?php  echo $cheme_txt;
		 ...
		 if($objResult["immunotherapy_follow"] == 'Yes' || $objResult["immunotherapy_follow"] =="Immunotherapy"){
		   echo "Yes";
		 }else if($objResult["immunotherapy_follow"] != ""){  echo "No"; } 
