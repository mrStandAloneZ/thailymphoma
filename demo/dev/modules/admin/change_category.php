...
<TABLE width="95%" align=center cellSpacing=0 cellPadding=0 border=0>
				<TR>
					<TD>
					<h1>หมวดหมู่การแลกเปลี่ยนเวร</h1>
                    &nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=change_category&op=change_add"><IMG SRC="images/admin/opendir.gif"  BORDER="0" align="absmiddle"> เพิ่มหมวดหมู่</A><BR><BR>
<?
//////////////////////////////////////////// แสดงรายการ
if($_GET[op] == ""){
?>
...
?>
    <tr>
     <td width="44">
      <a href="?name=admin&file=change_category&op=change_edit&id=<? echo $arr[boardcat][id];?>"><img src="images/admin/edit.gif" border="0" alt="แก้ไข" ></a> 
      <a href="javascript:Confirm('?name=admin&file=change_category&op=change_del&id=<? echo $arr[boardcat][id];?>','คุณมั่นใจในการลบหมวดหมู่นี้ ? เพราะจะทำการลบทุกหัวข้อในหมวดหมู่นี้ด้วย!!!');"><img src="images/admin/trash.gif"  border="0" alt="ลบ" ></a>
     </td> 
     <td><?echo $arr[boardcat][category_name];?></td>
	 <td align="center" width="50" ><?echo $row[sumboard] ;?></td>
     <td align="center" width="50"><A HREF="?name=admin&file=change_category&op=change_edit&action=sort&setsort=<?echo $SETSORT_UP ;?>&move=up&id=<? echo $arr[boardcat][id];?>"><IMG SRC="images/icon/arrow_up.gif"  BORDER="0" ALT="เลื่อนขึ้น"></A>&nbsp;&nbsp;&nbsp;<A HREF="?name=admin&file=change_category&op=change_edit&action=sort&setsort=<?echo $SETSORT_DOWN ;?>&move=down&id=<? echo $arr[boardcat][id];?>"><IMG SRC="images/icon/arrow_down.gif"  BORDER="0" ALT="เลื่อนลง"></A></td>
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการเพิ่มหมวดหมู่การแลกเปลี่ยน   เข้าสู่ระบบเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=change_category\"><B>กลับหน้า จัดการหมวดหมู่การแลกเปลี่ยน </B></A>";
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<BR><BR>";
		$ProcessOutput .= "<CENTER><A HREF=\"?name=admin&file=main\"><IMG SRC=\"images/login-redirection-loader.gif\" BORDER=\"0\"></A><BR><BR>";
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการแก้ไขหมวดหมู่การแลกเปลี่ยน   เข้าสู่ระบบเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=change_category\"><B>กลับหน้า จัดการหมวดหมู่การแลกเปลี่ยน </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	}else{
...
<?
	...
	if(CheckLevel($_SESSION['admin_user'],$_GET[op])){
		...
		$ProcessOutput .= "<BR><BR>";
		$ProcessOutput .= "<CENTER><A HREF=\"?name=admin&file=main\"><IMG SRC=\"images/login-redirection-loader.gif\" BORDER=\"0\"></A><BR><BR>";
		$ProcessOutput .= "<FONT COLOR=\"#336600\"><B>ได้ทำการลบหมวดหมู่การแลกเปลี่ยนเรียบร้อยแล้ว</B></FONT><BR><BR>";
		$ProcessOutput .= "<A HREF=\"?name=admin&file=change_category\"><B>กลับหน้า จัดการหมวดหมู่การแลกเปลี่ยน</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	}else{
