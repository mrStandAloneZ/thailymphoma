...
if (isset($user_login) and isset($pwd_login)) {
    ...
    $result = mysql_query("select user,password,hostpital_engname,codehos from " . TB_MEMBER . " where user='$user_login' and password='$pwd_login'");
    $num = mysql_num_rows($result);
	if($result){
	$data_m = mysql_fetch_array($result);
			  $_SESSION['s_codehos'] = $data_m["codehos"];
              $_SESSION['s_codehos_full'] = $data_m["hostpital_engname"];
			  $_SESSION['ty_user'] = 1;
	}
    if ($num <= 0) {
		?>
