...
if($_GET[action] == "comment"){
	...
	$db->add_db(TB_WEBBOARD_COMMENT,array(
	 ...
	 $timeupdate=time();
	 $db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
	 $db->update(TB_WEBBOARD," update_date='$timeupdate' "," id='$_GET[id]' ");
	 $db->closedb ();
...
if($_SESSION['admin_user']){
	if($VIEWBOARD[pin_date]){
		echo "<A HREF=\"javascript:Confirm('index.php?name=change&file=pin_topic&action=removepin&id=".$_GET[id]."','คุณมั่นใจในการลบกระทู้นี้ออกจากการปักหมุด ?');\"><IMG SRC=\"images/admin/pin.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=\"0\" ALIGN=\"absmiddle\"> ยกเลิกปักหมุด </A>&nbsp;&nbsp;&nbsp;";
	}else{
		echo "<A HREF=\"javascript:Confirm('index.php?name=change&file=pin_topic&action=addpin&id=".$_GET[id]."','คุณมั่นใจในการปักหมุดให้กระทู้นี้ ?');\"><IMG SRC=\"images/admin/pin.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=\"0\" ALIGN=\"absmiddle\"> ทำการปักหมุด </A>&nbsp;&nbsp;&nbsp;";
	}
	if($VIEWBOARD[enable]){
		echo "<A HREF=\"javascript:Confirm('index.php?name=change&file=enable&action=removepin&id=".$_GET[id]."','คุณมั่นใจในการลบกระทู้นี้ออกจากการยืนยัน  ?');\"><IMG SRC=\"images/admin/pin.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=\"0\" ALIGN=\"absmiddle\"> ยกเลิกยืนยัน  </A>&nbsp;&nbsp;&nbsp;";
	}else{
		echo "<A HREF=\"javascript:Confirm('index.php?name=change&file=enable&action=addpin&id=".$_GET[id]."','คุณมั่นใจในการยืนยันให้กระทู้นี้ ?');\"><IMG SRC=\"images/admin/pin.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=\"0\" ALIGN=\"absmiddle\"> กดยืนยัน </A>&nbsp;&nbsp;&nbsp;";
	}
	echo " <A HREF=\"javascript:Confirm('index.php?name=change&file=delete_topic&id=".$_GET[id]."','คุณมั่นใจในการลบกระทู้นี้ ?');\"><IMG SRC=\"images/admin/trash.gif\" WIDTH=\"20\" HEIGHT=\"20\" BORDER=\"0\" ALIGN=\"absmiddle\"> ลบกระทู้นี้ </A>";
};
?>
					<BR><BR>
					</TD>
...
<FORM name="form2" METHOD=POST ACTION="index.php?name=change&file=read&action=comment&id=<?=$_GET[id];?>" enctype="multipart/form-data" >
